...
function formatSecondsToTime(n) {
    const t = new Date(n * 1000).toISOString().slice(12, 23);
...
function addAboveSubtitle(i) {
    subtitlesList.splice(i, 0, {start: subtitlesList[i-1]?.end, end: subtitlesList[i]?.start, text: ''});
    renderSubtitles();
}

function addBelowSubtitle(i) {
    subtitlesList.splice(i+1, 0, {start: subtitlesList[i]?.end, end: subtitlesList[i+1]?.start, text: ''});
    renderSubtitles();
}
